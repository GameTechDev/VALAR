...
namespace Graphics
{
    ...
    ColorBuffer g_VRSTier2Buffer;
    //ReadbackBuffer g_ShadingRateReadbackBuffer;
...
void Graphics::InitializeRenderingBuffers( uint32_t bufferWidth, uint32_t bufferHeight )
{
    ...
    esram.PushStack();
        ...
        // VRS Tier 2
        VRS::CheckHardwareSupport();
        //if (VRS::ShadingRateTier == D3D12_VARIABLE_SHADING_RATE_TIER_2)
        {
            g_VRSTier2Buffer.Create(L"Shading Rate Image Buffer", (uint32_t)round(bufferWidth / (float)VRS::ShadingRateTileSize), (uint32_t)round(bufferHeight / (float)VRS::ShadingRateTileSize), 1, DXGI_FORMAT_R8_UINT);
            Color clearColor = Color((float)D3D12_SHADING_RATE_1X1, 0.0f, 0.0f);
            //g_VRSTier2Buffer.SetClearColor(clearColor);
            //InitContext.ClearColor(g_VRSTier2Buffer);
            InitContext.TransitionResource(g_VRSTier2Buffer, D3D12_RESOURCE_STATE_COPY_DEST);
            VRS::CreateSubresourceData();
            InitContext.TransitionResource(g_VRSTier2Buffer, D3D12_RESOURCE_STATE_SHADING_RATE_SOURCE);

           /* g_ShadingRateReadbackBuffer.Create(L"Readback Shading Rates",
                round((float)g_DisplayWidth / (float)VRS::ShadingRateTileSize) * round((float)g_DisplayHeight / (float)VRS::ShadingRateTileSize), sizeof(uint8_t));*/
        }
