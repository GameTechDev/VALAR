...
#include "CompiledShaders/DoFVRSFullRateCS.h"
#include "CompiledShaders/DoFVRSHalfRateCS.h"
#include "CompiledShaders/DoFVRSQuarterRateCS.h"
...
namespace DepthOfField
{
    ...
    ComputePSO s_DoFVRSFullRateCS(L"DOF: VRS Full Rate CS");				// Output Full VRS Rate 1x1
    ComputePSO s_DoFVRSHalfRateCS(L"DOF: VRS Half Rate CS");			    // Output Half VRS Rate 2x2
    ComputePSO s_DoFVRSQuarterRateCS(L"DOF: VRS Quarter Rate CS");			// Output Qarter VRS Rate 4x4
...
#define CreatePSO( ObjName, ShaderByteCode ) \
    ...
    CreatePSO(s_DoFVRSFullRateCS, g_pDoFVRSFullRateCS);
    CreatePSO(s_DoFVRSHalfRateCS, g_pDoFVRSHalfRateCS);
    CreatePSO(s_DoFVRSQuarterRateCS, g_pDoFVRSQuarterRateCS);
...
void DepthOfField::RenderVRSBuffer(ComputeContext& Context, bool LowQuality)
{
    Context.SetRootSignature(s_RootSignature);
    ...
    DoFConstantBuffer cbuffer =
    {
        ...
        1.0f / TiledWidth, 1.0f / TiledHeight,
        (uint32_t)DebugMode, EnablePreFilter ? 0u : 1u,
        ForegroundRange / 1, 1 / ForegroundRange, (float)AntiSparkleWeight,
        VRS::ShadingRateTileSize
    };
    Context.SetDynamicConstantBufferView(0, sizeof(cbuffer), &cbuffer);

    Context.ClearUAV(g_VRSTier2Buffer);

    if (LowQuality)
    {
        Context.SetPipelineState(s_DoFVRSHalfRateCS);
        Context.SetDynamicDescriptor(1, 5, g_DoFWorkQueue.GetSRV());
        Context.SetDynamicDescriptor(2, 0, g_VRSTier2Buffer.GetUAV());
        Context.DispatchIndirect(s_IndirectParameters, 0);

        Context.SetPipelineState(s_DoFVRSQuarterRateCS);
        Context.SetDynamicDescriptor(1, 5, g_DoFFastQueue.GetSRV());
        Context.SetDynamicDescriptor(2, 0, g_VRSTier2Buffer.GetUAV());
        Context.DispatchIndirect(s_IndirectParameters, 12);

        Context.SetPipelineState(s_DoFVRSFullRateCS);
        Context.SetDynamicDescriptor(1, 5, g_DoFFixupQueue.GetSRV());
        Context.SetDynamicDescriptor(2, 0, g_VRSTier2Buffer.GetUAV());
        Context.DispatchIndirect(s_IndirectParameters, 24);
    }
