...
namespace EngineProfiling
{
    ...
    void DisplayVRSInfo(TextContext& Text)
    {
        Text.SetColor(Color(0.5f, 1.0f, 1.0f));
        Text.DrawFormattedString("Hardware Details\n");
        Text.SetColor(Color(1.0f, 1.0f, 1.0f));
        Text.SetTextSize(18.0f);

        Text.DrawFormattedString("PSInvocations: %llu\n", VRS::PipelineStatistics.PSInvocations);

        //VRS Shading Rate %
        if (VRS::CalculatePercents)
        {
            Text.DrawFormattedString("1x1: %.2f%%  ", VRS::Percents.num1x1);
            Text.DrawFormattedString("1x2: %.2f%%  ", VRS::Percents.num1x2);
            Text.DrawFormattedString("2x1: %.2f%%\n", VRS::Percents.num1x2);
            Text.DrawFormattedString("2x2: %.2f%%  ", VRS::Percents.num2x2);
            Text.DrawFormattedString("2x4: %.2f%%  ", VRS::Percents.num2x4);
            Text.DrawFormattedString("4x2: %.2f%%\n", VRS::Percents.num4x2);
            Text.DrawFormattedString("4x4: %.2f%%\n", VRS::Percents.num4x4);
        }

        // VRS Tier
        if (VRS::ShadingRateTier == D3D12_VARIABLE_SHADING_RATE_TIER_NOT_SUPPORTED)
        {
            Text.DrawFormattedString("VRS is NOT supported");
            return;
        }
        else if (VRS::ShadingRateTier == D3D12_VARIABLE_SHADING_RATE_TIER_1)
        {
            Text.DrawFormattedString("VRS Tier 1 supported\n");
        }
        else if (VRS::ShadingRateTier == D3D12_VARIABLE_SHADING_RATE_TIER_2)
        {
            Text.DrawFormattedString("VRS Tier 1 and 2 supported\n");
            // Tile Size
            Text.DrawFormattedString("Tile size: %iX%i\n", VRS::ShadingRateTileSize, VRS::ShadingRateTileSize);
        }

        // Additional shading rates
        if (VRS::ShadingRateAdditionalShadingRatesSupported)
        {
            Text.DrawFormattedString("Additional shading rates supported\n");
        }
        else
        {
            Text.DrawFormattedString("Additional shading rates NOT supported\n");
        }

        Text.DrawFormattedString("(press Backspace to toggle UI)");
    }
